export const linearize = (p1, p2, rx, ry, angle, largearc, sweep, tol) => {
...
export const centerToSvg = (center, rx, ry, theta1, extent, angleDegrees) => {
...
export const svgToCenter = (p1, p2, rx, ry, angleDegrees, largearc, sweep) => {
  var mid = {
    x: 0.5 * (p1.x + p2.x),
    y: 0.5 * (p1.y + p2.y)
  };
...
const _withinDistance = (p1, p2, distance) => {
  var dx = p1.x - p2.x;
  var dy = p1.y - p2.y;
  return dx * dx + dy * dy < distance * distance;
};

const _degreesToRadians = angle => {
  return angle * (Math.PI / 180);
};

const _radiansToDegrees = angle => {
